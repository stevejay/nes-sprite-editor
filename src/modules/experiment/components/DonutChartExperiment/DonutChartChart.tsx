...
import { includes, sortBy, random } from "lodash";
...
function findArc(i: number, data0, data1, order, zeroAdjustRadians) {
  const d = data0[i];
  const dOrder = order(d);

  if (!data1.length) {
    return { startAngle: zeroAdjustRadians, endAngle: zeroAdjustRadians };
  }
  ...
  for (let i = 0; i < data1.length; ++i) {
    if (order(data1[i]) > dOrder) {
      ...
      if (i === 0) {
        return {
          startAngle: zeroAdjustRadians,
          endAngle: zeroAdjustRadians
        };
      } else {
        const d = data1[i - 1];
  ...
  return {
    startAngle: Math.PI * 2 + zeroAdjustRadians,
    endAngle: Math.PI * 2 + zeroAdjustRadians
  };
}

function findExitingArc(i: number, data0, data1, order, zeroAdjustRadians) {
  return findArc(i, data0, data1, order, zeroAdjustRadians);
}

function findEnteringArc(i: number, data0, data1, order, zeroAdjustRadians) {
  return findArc(i, data1, data0, order, zeroAdjustRadians);
}
...
class DonutChartChart extends React.Component<Props> {
  ...
  private renderGraph() {
    ...
    const zeroAdjustRadians = random(ZERO_ADJUST_RADIANS, 1, true);
    ...
    data1.forEach(d => {
      d.startAngle += zeroAdjustRadians;
      d.endAngle += zeroAdjustRadians;
    });
    ...
    path
      .exit()
      .datum(function(d, i) {
        const findNeighbour = findExitingArc(
          i,
          data0,
          data1,
          order,
    ...
    path = path
      ...
      .each(function(d, i) {
        this._current =
          findEnteringArc(i, data0, data1, order, zeroAdjustRadians) || d;
      })
    ...
    label
      .text(labelText)
      .each(
        d => (d.direction = midAngle(d) >= 0 && midAngle(d) < Math.PI ? 1 : -1)
      )
    ...
    polyline
      ...
      .attrTween("points", function(d) {
        ...
        return function(t) {
          const d2 = interpolate(t);
          const direction =
            midAngle(d2) >= 0 && midAngle(d2) < Math.PI ? 1 : -1;
          const pos = labelArc.centroid(d2);
